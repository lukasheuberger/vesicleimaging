...
def create_ipyviewer_ome_tiff(array, metadata):
    ...
    :return: out - interactive widgets for jupyter notebook
...
def create_ipyviewer_czi(cziarray, metadata):
    ...
    if sliders == 'BVCTZR':
        ui = widgets.VBox([b, c, t, z, r])

        def get_TZC_czi(b_ind, c_ind, t_ind, z_ind, r):
            display_image(cziarray, metadata, sliders, b=b_ind, t=t_ind, c=c_ind, vmin=r[0], vmax=r[1], z=z_ind)

        out = widgets.interactive_output(get_TZC_czi, {'b_ind': b, 't_ind': t, 'c_ind': c, 'r': r, 'z_ind': z})
...
def display_image(array, metadata, sliders,
    ...
    if metadata['ImageType'] == 'czi':
        ...
        if sliders == 'BVCTZR':
            # reduce dimensionality (V)
            image = np.squeeze(array, axis = (1)) # -> BCTZR
            if metadata['isRGB']:
                image = array[b - 1, c - 1, t - 1, z - 1, :, :, :]
                print ('rgb')
            else:
                image = array[b - 1, c - 1, t - 1, z - 1, :, :][0]
                print ('not rgb')
